<?php
/*
modifyied by abulhol for cwbc
*/

/*START CONFIGURATION SECTION*/
$dashboardPage = 'dashboard';
$dashboardPageSecret = 'abulhol';
...
<html>
<head>
    <title>CWBC Email Pixel Dashboard</title>
...
var Dashboard = {
  documentTitle : 'Cwbc Email Pixel Dashboard',
  ...
  loadTrackingReports : function(update){
      Dashboard.services.callLoadTrackingReportService($('#trackUUIDTxt').val(), function(trackingReportJson){
          ...
          var newTracksCounter = 0;
		   data =  trackingReportJson.trackList.filter((obj, pos, arr) => {
            return arr.map(mapObj =>
                  mapObj.ip).indexOf(obj.ip) == pos;
            });
			trackingReportJson.trackList=data ;
          trackingReportJson.trackList.forEach(function(track, trackIndex){
           if(trackIndex < $('#reportsDiv').children().length || track.headers['User-Agent'].indexOf('GoogleImageProxy') > -1)
                  return;
              newTracksCounter++;
              
              if( newTracksCounter!=0)
                  document.title = '('+newTracksCounter+') '+Dashboard.documentTitle;
			   $("#opensofar").show();

			      $("#opensofar").text("Email Open So Far " + newTracksCounter);
...
$(document).ready(Dashboard.initialize);
 // $('#reportsDiv').append("<b>Email open so far</b>");
 $("#opensofar").hide();
    </script>
    <style type="text/css">
@charset "UTF-8";
...
<body>
    <div id="mainContainer" class="container">
        <div class="page-header text-center">
            <h1>CWBC - Email Pixel Tracker </h1><h1><small>Dashboard</small></h1>
        </div>
...
<div class="alert alert-success" STYLE=" display: none;" role="alert" id="opensofar">
  !
</div>
